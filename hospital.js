...
class Hospital {
    constructor(name, location) {
        ...
        this.employees = ['Andi','Rian','Agung','Ari','Patient']
        this.username = ['andi','rian','agung','ari','patient']
        this.position = ['Doctor','Admin','Receptionist','Officeboy','Patient']
        this.patients = [{id:1, name:"Kamil", diagnose:"flu"},{id:2, name:"Ahmad", diagnose:"batuk"}]
        this.id = 0
  }
  ...
  login(){
      rl.question('\n\nPlease enter your username (hint : andi / rian / agung / ari / patient)\nAnswer : ', (answer) => {
          if (this.username.indexOf(answer) !== -1 ) {
              this.id = this.username.indexOf(answer)
              this.password();
          }else {
            console.log('\n\n Try again!. Invalid username !.\n');
            this.login();
          }
      });
  }
  ...
  password(){
      rl.question('\n\nPlease enter your Password (use your username)\nAnswer : ', (answer) => {
          if (answer === this.username[this.id]) {
              this.welcome()
          }else {
  ...
  view_records(){
   rl.question('\nEnter patient ID: (type exit to back to home page)', (answer) => {
 ...
 add_records(){
  ...
  option(){
      rl.question('\nEnter option or exit to logout:\ninput : ', (answer) => {
          switch (answer) {
              case 'list_patients':
                  this.list_patients()
                  break;
...
let hospital = new Hospital('Didit','Jakarta')
hospital.header()
